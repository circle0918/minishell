...
/*   Updated: 2022/01/14 00:42:53 by thhusser         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		count_word(t_ms *g, char *line)
{
	int		i;
	int		nb;
...
char	**creat_list_arg2(t_ms *g, char **cmd, int i, int j)
{
	int		k;
	...
	while (g->line[i])
	{
		if (g->line[i] == ' ')
		{
			...
			if (g->line[i] == '\0')
				break ;
			cmd[j] = (char *)ft_calloc(sizeof(char), (ft_strlen(&g->line[i]) + 1));
			if (!cmd[j])
				return (NULL);
		}
		cmd[j] = get_cmd_creat(cmd[j], g->line, &i, &k);
		i++;
	}
...
char	**creat_list_arg(t_ms *g, char *line)
{
	...
	command = (char **)malloc(sizeof(char *) * (count_word(g, line) + 1));
	if (!command)
		return (NULL);
	command[0] = (char *)ft_calloc(sizeof(char), (ft_strlen(line) + 1));
	if (!command)
		return (NULL);
	command = creat_list_arg2(g, command, 0, 0);
	// print_split(command);
	return (command);
}
